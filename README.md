...
本リポジトリでは、音声データを以下のように管理します：

- **大容量データ（Git管理外）**
  - `data/raw/` - LibriSpeech元データ（.gitignoreで除外）
  - `data/processed/` - 加工済みデータ（.gitignoreで除外）
  - これらは各開発者が個別にダウンロード・生成する

- **サンプル音声（Git管理対象）**
  - `EnglishListeningApp/EnglishListeningApp/Resources/AudioSample/` - MVP動作確認用の少量サンプル
  - リポジトリに含めることで、即座にビルド・動作確認が可能
  - ファイルサイズは最小限に抑える（数MB程度）
